...
{
    // todo: change to predefined macro
    memset(id, 0, 16);
...
bool DroneCANInterface::broadcast(const Canard::Transfer &bcast_transfer) {
    tx_transfer = {
...
bool DroneCANInterface::request(uint8_t destination_node_id, const Canard::Transfer &req_transfer) {
    tx_transfer = {
...
bool DroneCANInterface::respond(uint8_t destination_node_id, const Canard::Transfer &res_transfer) {
    tx_transfer = {
...
void DroneCANNode::handle_GetNodeInfo(const CanardRxTransfer& transfer, const uavcan_protocol_GetNodeInfoRequest& req)
{
    ...
    // fill in node name
    node_info_rsp.name.len = snprintf((char*)node_info_rsp.name.data, sizeof(node_info_rsp.name.data), "DroneCANNode");
 ...
 */
void DroneCANNode::handle_DNA_Allocation(const CanardRxTransfer& transfer, const uavcan_protocol_dynamic_node_id_Allocation& msg)
{
 ...
 */
void DroneCANNode::send_NodeStatus(void)
{
...
*/
void DroneCANNode::process1HzTasks(uint64_t timestamp_usec)
{
    /*
...
*/
void DroneCANInterface::process(uint32_t timeout_msec)
{
    // Transmitting
 ...
 */
void DroneCANInterface::onTransferReceived(CanardInstance* ins, CanardRxTransfer* transfer)
{
    DroneCANInterface* iface = (DroneCANInterface*) ins->user_reference;
 ...
 */
bool DroneCANInterface::shouldAcceptTransfer(const CanardInstance* ins,
...
*/
void DroneCANInterface::init(const char *interface_name)
{
    // todo: use predefined Macro instead
    const uint32_t peripheral_clock_rate = HAL_RCC_GetPCLK1Freq();
    const uint32_t target_bitrate = 1000000;
    int16_t res = canardSTM32ComputeCANTimings(peripheral_clock_rate, target_bitrate, &timings);
    if (res < 0) {
        (void)fprintf(stderr, "Failed to compute CAN timing '%s'\n", interface_name);
        exit(1);
    }
